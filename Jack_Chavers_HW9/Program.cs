// Jack Chavers
// MIS 3013 HW 5 Part 3 Dictionary
...
while(userInput=="yes"||userInput=="y")
{
    ...
    Console.Write(m);
    string balancesStr = Console.ReadLine();
    double balance = Convert.ToDouble(balancesStr);
...
{
    m = string.Format($"Name:{clientDict[userInput].name}, ID: {clientDict[userInput].id}, Balance: {clientDict[userInput].balance}, Age: {clientDict[userInput].age}, Weight {clientDict[userInput].weight:F2}");
    Console.WriteLine(m);
}
else
{
